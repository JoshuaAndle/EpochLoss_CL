...
def load_task_paths(args=None):
    ...
    #!# Will try to streamline this down the road into a more unified pathing
...
def prepare_allbatches(sortMethod=None, set_size=10, dataset=None):
    ...
    ### Dont use these unless you also shuffle extraloader accordingly
    # if sortMethod == "sorted":    
    #     ### Get the sorted indices based on labels
    #     new_indices = torch.argsort(dataset['y'])
    # elif sortMethod == "random":
    #     ### Randomly shuffle based on trial/seed
    #     new_indices = torch.randperm(len(dataset['y']))
    # else:e        0:08  8-08:00:00        cpu=2,mem=128G,node=1,billing=2,gres/gpu=1        gres:gpu:1         node-g102
