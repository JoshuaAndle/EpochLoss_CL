...
def generate_activations(data_loader, model, device, item_key, save_act=False, save_act_fname='norm', save_id_fname='idnorm', ret_act=True, mid_level=False, f_idx=None, sampledict=None, perturbed=False, trial=-1):
    ...
    with torch.no_grad():
        for step, data in enumerate(data_loader):
            ...
            # if step == 0:
            #     print("IDs first step of trial: ", z_label)
            #     print("Size of x_input: ", x_input.shape)
            ## Trial for x_input squared
            ...
            # ### Reshape to 4 dimensions then broadcast to the shape of the activations for consistency
            # id_op = id_op[:, None, None, None]
            # id_op = np.broadcast_to(id_op, temp_op.shape)
 
            # if step == 0:
            #     print("temp op shape: ", temp_op.shape)
                # print("Id_op shape: ", id_op.shape, " ", id_op[0])
...
#                 temp_pred_labels_op = pred.cpu().numpy()
...
#                 temp_pred_labels_op = np.hstack((pred.cpu().numpy(), temp_pred_labels_op))
...
#                     pred_labels_op  = copy.deepcopy(temp_pred_labels_op)

#                     temp_op        = None
#                     temp_labels_op = None
#                     temp_pred_labels_op = None

#                 else:
#                     parents_op = np.vstack((temp_op, parents_op))
#                     labels_op  = np.hstack((temp_labels_op, labels_op))
#                     pred_labels_op  = np.hstack((temp_pred_labels_op, pred_labels_op))
...
#     return parents_op, labels_op, pred_labels_op
