...
def process_images(num_classes, aiClassDict, natureClassDict, args, target_size=(64, 64)):
    ...
    generator_classes = classDict[generator]
    ...
    for imageset in ['train', 'val']:
        ...
        for imagetype in ['ai', 'nature']:
            ...
            joint_folder = os.path.join(args.folder_path, generator, "joint", imageset, imagetype)
            disjoint_folder = os.path.join(args.folder_path, generator, "disjoint", imageset, imagetype)
            ...
            if not os.path.exists(disjoint_folder):
                os.makedirs(disjoint_folder, exist_ok=True)
            ...
            for filename in os.listdir(input_folder):
                ...
                # ### Check if this class is in the joint subset
                # if classString in joint_classes:
                #     input_path = os.path.join(input_folder, filename)
                #     output_path = os.path.join(joint_folder, newFileName)

                #     try:
                #         # Open, resize, and save the image
                #         with Image.open(input_path) as img:
                #             ### The nature images are irregular resolutions, so resizing will be handled during tensor processing
                #             if imagetype == "ai":
                ...
                if classString in generator_classes:
                    input_path = os.path.join(input_folder, filename)
                    output_path = os.path.join(disjoint_folder, newFileName)
