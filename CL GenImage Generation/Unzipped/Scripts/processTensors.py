...
FLAGS.add_argument('--output_folder', type=str, default="processed", help='Id of current run.')
...
def main():
    ...
    # for expType in ['joint', 'disjoint']:
    for expType in ['disjoint']:
...
def images_as_tensor(generator, expType, split, imgType, reverseDict, args, target_size=64):
    ...
    directory = os.path.join(".", generator, expType, split, imgType)
    saveDirectory = os.path.join(".", args.output_folder, generator, expType, split, imgType)
